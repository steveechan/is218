...
console.log(slug)
  return (
    <Popover className="relative text-right">
      {({ open }) => (
        <>
          <Popover.Button className={`flex items-center gap-2 p-2 focus:outline-none  rounded ${router.asPath.includes(slug) ? "font-semibold" : null}`}>
            {name} {open ? <AiFillCaretUp /> : <AiFillCaretDown />}
          </Popover.Button>
          <Popover.Panel className="absolute min-w-[200px] right-0 mt-2 z-20">
