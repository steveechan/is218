...
const BlogPage = ({ blogData }) => {
  const { author, date, title,description } = blogData;
  return (
    <>
        <MetaHeader title={title} description={description} ogImage={`${BASE_URL}/api/og?title=${title}&description=${description}`} />
        <PageLayout>
      <section className="container mx-auto  max-w-[65ch]  my-12 space-y-24 p-4">
        <div className="space-y-8 overflow-hidden">
          ...
          <img
            ...
            className="absolute object-cover -z-10 top-[0] right-0 h-[250px] w-screen opacity-25 select-none"
          />
        </div>
